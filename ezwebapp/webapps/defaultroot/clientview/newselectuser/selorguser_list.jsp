              ...
              <div class="step-todo step-select">
                  ...
                  <div class="tabs" id="orgTabs">
                                  ...
                                  <ul>
                                  	...
                                  	<x:parse xml="${docXml}" var="personDoc"/>
										...
										<x:forEach select="$personDoc//list" var="n" varStatus="status">
											...
											<li id="userLi_${empId }" data-empid="$${empId }$" data-empname="${empName }," <c:if test="${sex==0 || sex ==-1}">class="man"</c:if> <c:if test="${sex==1}">class="lady"</c:if>>
	                                          <a href="">
	                                              <span>${empName }</span>
	                                          </a>
	                                      </li>
                      ...
                      <div id="userList" class="tab">
                          <div class="select-top">
                              <div class="my-part-tree my-part-treeo list-block">
                                  <ul id="userListLi">
                                  </ul>
...
<script type="text/javascript">
	...
	$(function(){
		...
		var groupFlag = sessionStorage.getItem("selGroupId_"+selectId);
		if(groupFlag != null &&　groupFlag　!=""){
			selGroupId = groupFlag;
		}
	...
	function Selected(){
		$("#selectedPer").empty();
		var newStr1 = rangeSelectIds.replace(/(\\$\\$)/g,'$,$');
		...
		var str22 = selectNameVal.split(",");
		for(var i=0;i<str11.length;i++){
			if(str11[i].indexOf("$")>-1){
				selectUserIds+=str11[i];
			}else if(str11[i].indexOf("*")>-1){
				orgHtml += "<li class='swipeout'><i class='fa fa-check-circle fa-check-circle-active' name='"+str11[i]+"' data-empid='"+str11[i]+"' data-empname='"+str22[i]+",'></i>"
							+"<a href='###' class='swipeout-content item-content'>"
							+"<div>"
							+"<span class='people-name' style='font-size: 1rem;'>"+str22[i]+"</span>"
							+"</div></a></li>";
			}
		}
		$("#selectedPer").append(orgHtml);
		loadSelectedPer(selectUserIds);
	               	...
	               	if(data){
		                ...
		                if(personArray){
						    ...
						    for(var i = 0; i < personArray.length; i++){
								...
								result += "<li  onclick='checkYxUser(this,"+personArray[i].userId+",\""+personArray[i].userName+"\");' class='swipeout'><i class='fa fa-check-circle fa-check-circle-active' name="+personArray[i].userId+","+" data-empid="+personArray[i].userId+","+" data-empname="+personArray[i].userName+","+"></i>";
        		...
        		if(data){
	                ...
	                if(orgArray){
		                ...
		                for(var i = 0; i < orgArray.length; i++){
		                	...
		                	var checkHtml = '<div  id='+orgDivId+' onclick="checkSelOrg(this);" data-orgid='+'*'+orgArray[i].orgId+'*'+' data-orgname='+orgArray[i].orgName+','+'   class="select-check"></div>';
		...
		$(".my-part-tree ul li").click(function(){
	    	  ...
	    	  if(selectType == '0'){//单选
	    		...
	    		if($$(this).hasClass("checked")){
	    			...
	    			rangeSelectIds = rangeSelectIds.replace(empId,'');
	    		}else{
	    			$$(this).addClass("checked");
			        if(selectIdVal.indexOf(empId) == -1){
		    			selectIdVal += empId;
		    			selectNameVal += empName;
		    			rangeSelectIds += empId;
			        }
	    		}
	    	}
		})
	...
	function setSelectVal(){
		...
		if(idValArray){
			for(var i=0,length=idValArray.length; i<length; i++){
				if(idValArray[i].indexOf("$")!=-1){
					var id = idValArray[i].replace('$','').replace('$','');
					$(".userLi_"+id).addClass('checked');
				}
				if(idValArray[i].indexOf("*")!=-1){
					var id = idValArray[i].replace('*','').replace('*','');
					$(".divOrg_"+id).addClass('sel-kik');
				}
			}
		}
		$("#selectedNum").html("已选"+selectedNum);
	...
	function confirmSelect(){
		...
		sessionStorage.setItem("selGroupId_"+selectId,selGroupId);
	...
	function checkSelOrg(obj){
		...
		if($$(obj).hasClass("sel-kik")){
			...
			rangeSelectIds = rangeSelectIds.replace(orgId,'');
  	    }else{
		...
		$.ajax({
            ...
            success: function(data){
        		if(data){
	                ...
	                var personLis = '';
                	if(personArray){
                		personLis = '<div class="select-top"><div class="my-part-tree my-part-treeo list-block"><ul id='+ulId+'>';
	...
	function checkSelGroup(obj,groupId,nowTab,liGroupName){
		if($$(obj).hasClass("sel-kik")){
			  ...
			  selGroupId = selGroupId.replace(repId,'');
			  loadGruopPeopleData(groupId,obj,nowTab,liGroupName,2);
	          $$(obj).removeClass("sel-kik");
	      }else{
	    	  selGroupId += 'groupSel'+groupId + ",";
 ...
 // 清除搜索区域焦点bug
    ...
    function setCheckBox(){
		if(selGroupId != null && selGroupId != ""){
			var groupIdValArray = selGroupId.split(',');
			if(groupIdValArray){
