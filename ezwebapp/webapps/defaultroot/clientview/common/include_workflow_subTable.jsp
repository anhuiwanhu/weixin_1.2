...
<script type="text/javascript" src="/defaultroot/clientview/template115/js/fx.js"></script>
...
<section class="wh-section wh-section-topfixed wh-section-bottomfixed wh-section-morebtmfixed" id="subSection_${tableName}" style="display:none;position: absolute;top: 70px;bottom: 50px;padding-bottom: 0;padding-top: 0;overflow-y: scroll;-webkit-overflow-scrolling: touch; ">
					                            ...
					                            <td>                             
													<c:choose>
														...
														<c:when test="${showtype =='216' && readwrite =='1'}">
															<input type="hidden" readonly="readonly" id='_sub_<x:out select="$field/sysname/text()"/>_${subFieldsNum}' name='_sub_<x:out select="$field/sysname/text()"/>' value='<x:out select="$field/hiddenval/text()"/>' />
								           					<input type="text"   readonly="readonly" id='_mainShow_<x:out select="$field/sysname/text()"/>_${subFieldsNum}'  name='_mainShow_<x:out select="$field/sysname/text()"/>' value='<x:out select="$field/value/text()"/>' class="edit-ipt-r edit-ipt-arrow" onclick='selectUser("1","_mainShow_<x:out select="$field/sysname/text()"/>_${subFieldsNum}","_sub_<x:out select="$field/sysname/text()"/>_${subFieldsNum}","*0*","orguser");' placeholder="请选择"/>
														</c:when>
...
<footer class="wh-ofooter" id="subFooter_totname" style="display:none;z-index:99999">
    <div class="wh-wrapper">
        <div class="wh-container">
...
<footer class="wh-footer wh-footer-forum zibiao-foot" id="subFooter_${tableName}" style="display:none;">
    <c:choose>
...
<script type="text/javascript">
    		...
    		$(this).find('input,select,textarea').each(function(){
    			...
    			if(oldId){
	    			...
	    			if(oldId.substring(oldId.length-1) == '_'){
						var newval = oldId.split('_')[0]+'_'+oldId.split('_')[1]+'_'+oldId.split('_')[2]+'_'+tableIndex+'_';//解决点击完成后再添加id变化引起冲突
						var newval2 = oldId.split('_')[0]+'_'+oldId.split('_')[1]+'_'+tableIndex+'_';						
						if(oldId.split('_').length==5){
							$(this).attr('id',newval);
						}
						if(oldId.split('_').length==4){
							$(this).attr('id',newval2);
						}
	    				return;
	    			}
   ...
   //图片数标记
	...
	function setCheckBoxVal(obj){
		...
		var $hideInput = $checkBox.parent().parent().next('input');
...
　　}
	...
	//主表合计字段
	function maniSum(){
		var $elements = $('.edit-ipt-r.mainhj');
		var len = $elements.length;
		$elements.each(function() {
			var $this = $(this);
			var val = $this.attr("mainsum");//主表合计字段子表字段的字段名
			val = "_sub_"+val;
			var sum= 0-0;
			...
			var rmb = changeNumMoneyToChinese(sum);
			$("#"+newId).val(rmb);
		});
	}
