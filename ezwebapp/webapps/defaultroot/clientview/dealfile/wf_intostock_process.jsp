                ...
                <div id="tab11" class="tab active">
                  <div class="form-table">
                    <table style="border-collapse:separate;border-spacing: 0 10px;">
                      <!-- 1:入库  4:退货 -->
		            	<c:set var="ptMode"><x:out select="$doc//fieldList/ptMode/text()"/></c:set>
		            	<!--入库单号/退货单号-->
		            	...
		            	<!-- 入库类别ID/退货类别ID -->
		            	<c:set var="typeId"><x:out select="$doc//fieldList/typeId/text()"/></c:set>
		            	<!-- 入库类别名称/退货类别名称 -->
		            	<c:set var="typeName"><x:out select="$doc//fieldList/typeName/text()"/></c:set>
		            	<!-- 供货单位名称/退货单位名称 -->
		            	<c:set var="suppName"><x:out select="$doc//fieldList/suppName/text()"/></c:set>
		            	<!-- 入库日期/退库日期 -->
		            	<c:set var="ptDate"><x:out select="$doc//fieldList/ptDate/text()"/></c:set>
		            	<!-- 经手人 -->
		            	<c:set var="handleName"><x:out select="$doc//fieldList/handleName/text()"/></c:set>
		            	<!-- 发票号码-->
		            	<c:set var="invoiceNo"><x:out select="$doc//fieldList/invoiceNo/text()"/></c:set>
		            	<!-- 备注 -->
		            	<c:set var="remark"><x:out select="$doc//fieldList/remark/text()"/></c:set>
	           			<tr>
							<td>
								<c:if test="${ptMode == '1'}"><span>入库仓库:</span></c:if>
								<c:if test="${ptMode == '4'}"><span>退货仓库:</span></c:if>
							</td>
							<td>${stockName }</td>
						</tr>
						<tr>
							<td>
								<c:if test="${ptMode == '1'}"><span>入库部门:</span></c:if>
								<c:if test="${ptMode == '4'}"><span>退货部门:</span></c:if>
							</td>
							<td>${orgName } </td>
						</tr>
						<tr>
							<td>
								<c:if test="${ptMode == '1'}"><span>入库类别:</span></c:if>
								<c:if test="${ptMode == '4'}"><span>退货类别:</span></c:if>
							</td>
							<td>${typeName }</td>
						</tr>
						<tr>
							<td>
								<c:if test="${ptMode == '1'}"><span>供货单位:</span></c:if>
								<c:if test="${ptMode == '4'}"><span>退货单位:</span></c:if>
							</td>
							<td>${suppName }</td>
						</tr>
						<tr>
							<td>
								<c:if test="${ptMode == '1'}"><span>入库日期:</span></c:if>
								<c:if test="${ptMode == '4'}"><span>退货日期:</span></c:if>
							</td>
							<td>${ptDate }</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td>
								<div><input id="subTableInput" placeholder="添加子表" type="text" class="edit-ipt-r edit-ipt-arrow" 
								<c:if test="${not empty subTable}">value="${subTable}条子表数据"</c:if>
								 readonly="readonly" onclick="addSubTable();"/></div>
							</td>
						</tr>
						<!--子表信息end-->
                      <!--批示意见begin-->
