    ...
    <div class="view">
      <div class="pages">
        <div class="page" data-page="page-webapp-form">
            ...
            <div class="webapp-form">
              <div class="app-tabheader-line clearfix">
                <div class="clearfix">
                  ...
                  <a href="#tab13" class="tab-link tab-link-rw" style="width:50%" onclick="openWorkFlowGetLog();">
                    <span>流程记录</span>
                  </a>
                </div>
              </div>
              <div class="tabs">
                ...
                <div id="tab11" class="tab active">
                  <div class="form-table">
					<form id="sendForm" action="/defaultroot/workflow/sendnew.controller?modibutton=<%=(String)pageContext.getAttribute("modibutton") %>&isDossier=<%=(String)pageContext.getAttribute("isDossier") %>&govType=receivefile" method="post"  >
							...
							<input type="hidden" id="pdfgwlx" name="pdfgwlx" value="0"/>
							...
							<table style="border-collapse:separate;border-spacing: 0 5px;">
								<%
									...
									for(int i=0;i<govDocFormList.size();i++){
										...
										if(("receiveDate".equals(_map.get("keyId"))||
											"createdTime".equals(_map.get("keyId"))) && hiddenFieldString.indexOf((String)_map.get("sourcecode"))==-1){     //来文日期、创建日期
												...
												<tr>
													...
													<td>
                                                      <div class="edit-ipt-a-arrow">
														<input placeholder="选择日期"  class="edit-ipt-r edit-ipt-arrow" type="text" id='sys_<%=_map.get("keyId")%>' name='sys_<%=_map.get("keyId")%>' value='<%=_map.get("text")%>' onfocus="selectDateNew(this)" readonly/>
							...
							<%
										...
										}else if("fileUnit".equals(_map.get("keyId")) && hiddenFieldString.indexOf((String)_map.get("sourcecode"))==-1){     //来文单位
							                if("true".equals(_map.get("canEdit"))){
							%>	
												<tr>
													<td><span><%if("1".equals(_map.get("fieldIsNull"))){out.print("<em>*</em>");}%><%=_map.get("name")%></span></td>
													<td>
													   <div>
                                                        <input class="edit-ipt-r" type="text" placeholder="请输入" style="width: 70%;" id='sys_<%=_map.get("keyId")%>'  name='sys_<%=_map.get("keyId")%>' value='<%=_map.get("text")==null?"":_map.get("text")%>'><a style="border: 1px solid #3eaeff;padding: 1px 10px;border-radius: 12px;color: #3eaeff;" onclick='selectFileUnit("sys_<%=_map.get("keyId")%>");'>选择</a>
													</td>
												</tr>
							<%
											}else{
							...
							<%
										...
										}else if("fileNumber".equals(_map.get("keyId")) && hiddenFieldString.indexOf((String)_map.get("sourcecode"))==-1){   //文号
                                            if("true".equals(_map.get("canEdit"))){
							%>	
												<tr>
													<td><span><%if("1".equals(_map.get("fieldIsNull"))){out.print("<em>*</em>");}%><%=_map.get("name")%></span></td>
							...
							<%
										...
										}else if("title".equals(_map.get("keyId")) && hiddenFieldString.indexOf((String)_map.get("sourcecode"))==-1){    //标题
                                            if("true".equals(_map.get("canEdit"))){
							%>	
												<tr>
													<td><span><%if("1".equals(_map.get("fieldIsNull"))){out.print("<em>*</em>");}%><%=_map.get("name")%></span></td>
							...
							<%
										...
										}else if(("safetyGrade".equals(_map.get("keyId"))||
											     "zjkyType".equals(_map.get("keyId"))||
											     "zjkykeepTerm".equals(_map.get("keyId"))||
											     "fileType".equals(_map.get("keyId"))||
											     "field4".equals(_map.get("keyId"))) && hiddenFieldString.indexOf((String)_map.get("sourcecode"))==-1){     //秘密级别、文种、保管期限、文件归类、紧急程度
                                            if("true".equals(_map.get("canEdit"))){
							%>	
												<tr>
													<td><span><%if("1".equals(_map.get("fieldIsNull"))){out.print("<em>*</em>");}%><%=_map.get("name")%></span></td>
													<td>
													  <div class="edit-direct">
														...
														<select onchange="setSpanHtml(this);" class="btn-bottom-pop" id='sys_<%=_map.get("keyId")%>' name='sys_<%=_map.get("keyId")%>'>
															<option value="">请选择</option>
															<%
															  String datavalue = _map.get("datavalue")+"";
							                                  String text = _map.get("text")+"";
															  if(datavalue!=null){
																  String[] datavalues = datavalue.split(";");
																  if(datavalues.length>0){
																	 String selected = "";
																     for(int kk=0;kk < datavalues.length;kk++){
																		 selected = "";
																		 if(text.equals(datavalues[kk])) selected = " selected='true' ";
																	     out.print("<option value='"+datavalues[kk]+"' "+selected+">"+datavalues[kk]+"</option>");
																	 }
																  }
															  }
															%>
							...
							<%
										...
										}else if("quantity".equals(_map.get("keyId")) && hiddenFieldString.indexOf((String)_map.get("sourcecode"))==-1){      //份数
								              if("true".equals(_map.get("canEdit"))){
							%>	
												<tr>
													<td><span><%if("1".equals(_map.get("fieldIsNull"))){out.print("<em>*</em>");}%><%=_map.get("name")%></span></td>
							...
							<%
										...
										}else if("doComment".equals(_map.get("keyId")) && hiddenFieldString.indexOf((String)_map.get("sourcecode"))==-1){       //批示内容
                                            if("true".equals(_map.get("canEdit"))){
							%>	
												<tr>
													<td><span><%if("1".equals(_map.get("fieldIsNull"))){out.print("<em>*</em>");}%><%=_map.get("name")%></span></td>
							...
							<%
							 ...
							 %>
									<tr>
										...
										<td>
										  <div>
											<input type="hidden" readonly="readonly" id='cust_<%=fieldname%>' name='cust_<%=fieldname%>' value='<%=content%>' />
							  ...
							  <%
								...
								<c:if test="${not empty govDocXml}">
									...
									<x:forEach select="$govDoc//baseData" var="bd" >
										...
										<c:choose>
											<c:when test="${not empty commentFieldName && not empty commentField && '-1' ne commentField && 'nullCommentField' ne commentField && param.workStatus ne '102' && param.workStatus ne '101'&&param.workStatus ne '2'}">
												<tr>
													<td>
											...
											<c:when test="${empty commentFieldName && not empty workcurstep && not empty commentField && '-1' ne commentField && 'nullCommentField' ne commentField && param.workStatus ne '102' && param.workStatus ne '101'&&param.workStatus ne '2'}">
												<tr>
													<td>
	      ...
	      <section id="selectContentUnit" style="display:none" class="section-select-name wh-section wh-section-bottomfixed show">
	</section>
...
<script type="text/javascript" src="<%=rootPath%>/clientview/js/ajaxfileupload.js"></script>
  ...
  //解决input输入框的问题
    ...
    var workfolwSendFlag = 1;//防止退回重复提交
    function subForm(){
        confirmForm();
    ...
    //打开选择来文单位
	function selectFileUnit(selectId){ 
		myApp.showPreloader('页面加载中...');
		var selectIdVal = $('input[id="'+selectId+'"]').val();
        
		var postUrl = '/defaultroot/doc/selectFileUnit.controller';
		$.ajax({
			url : postUrl,
			type : "post",
			data : {'selectId':selectId},
			success : function(data){
				$("#selectContentUnit").append(data);
				hiddenContentUnit(0);
				myApp.hidePreloader();
			}
		});
	...
	function selectCallBackUnit(obj1,obj2){
	...
	function hiddenContentUnit(flag){
		if(flag == 0){
			if($('#mainContent').is(':hidden')){
			}else{
				$("#mainContent").css("display","none");
			...
			$("#selectContentUnit").css("display","block");
		}else if(flag == 1){
			$("#selectContentUnit").css("display","none");
				$("#mainContent").css("display","block");
				$("#footerButton").css("display","block");
			$("#selectContentUnit").empty();
		}
	}
	...
	function signPdf(realName,saveName,path,pdfpzr,pdfpzhj,pdfid){
         ...
         $.ajax({
			...
			//		saveName = "http://demo1.whir.net:50645/defaultroot/upload/pdffile/201707.pdf";
