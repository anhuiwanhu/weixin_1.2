					...
					<form id="sendForm" action="/defaultroot/workflow/sendnew.controller?modibutton=<%=(String)pageContext.getAttribute("modibutton") %>&isDossier=<%=(String)pageContext.getAttribute("isDossier") %>&govType=sendfile" method="post">
												...
												<tr>
													...
													<td>
                                                      <div class="edit-ipt-a-arrow">
														<input placeholder="选择日期"  class="edit-ipt-r edit-ipt-arrow" type="text" id='sys_<%=_map.get("keyId")%>' name='sys_<%=_map.get("keyId")%>' value='<%=_map.get("text")==null?"":_map.get("text")%>' onfocus="selectDateNew(this)" readonly/>
													  ...
													  <div class="edit-direct">
														...
														<select onchange="setSpanHtml(this);" class="btn-bottom-pop" id='sys_<%=_map.get("keyId")%>' name='sys_<%=_map.get("keyId")%>'>
															...
															<%
															  ...
															  if(datavalue!=null&&!"null".equals(datavalue)&&!"".equals(datavalue)){
							...
							<%
							 ...
							 %>
							        <tr>
										...
										<td>
										  <div>
											<textarea name='sys_<%=_map.get("keyId")%>' id='sys_<%=_map.get("keyId")%>' rows="4" onkeyup="$(this).next('.edit-form-num').html($(this).attr('maxlength')-$(this).val().length);" class="edit-txta edit-txta-l" maxlength="500"><%=_map.get("text")==null?"":_map.get("text")%></textarea>
											<span class="edit-form-num"><script>document.write(500-$('#sys_<%=_map.get("keyId")%>').val().length);</script></span>
										  </div>
										</td>
									</tr>
							  <%
							...
							%>	
												<tr>
													...
													<td>
													   <div>
														<input type="hidden" readonly="readonly" id='sys_hid_<%=_map.get("keyId")%>' name='sys_hid_<%=_map.get("keyId")%>' value='' />
														<input type="text"  id='sys_<%=_map.get("keyId")%>'  name='sys_<%=_map.get("keyId")%>' value='<%=_map.get("text")==null?"":_map.get("text")%>' class="edit-ipt-r edit-ipt-arrow" onclick='selectUser("1","sys_<%=_map.get("keyId")%>","sys_hid_<%=_map.get("keyId")%>","*0*","orguser");' placeholder="请选择"/>
													  </div>
													</td>
												</tr>
							<%
