        ...
        <div class="page" data-page="page-webapp-focum">
          ...
          <section class="section-focum wh-section wh-section-bottomfixed">
            <div class="webapp-focum-view">
              <table style="border-collapse:collapse; border-spacing:0; " id="tbody1">
            ...
            <div class="wh-wrapper">
              <div class="wh-container">
                <div class="footer-focum wh-footer-btn row no-gutter">
                  <a href="##" class="fbtn-matter col-50" id="hrf3" onclick="change('3')">
                    <div><i class="fa fa-forum-home"></i></div><span>论坛门户</span></a>
                  <a href="##" class="fbtn-cancle col-50" id="hrf4" onclick="change('4')">
                    <div><i class="fa fa-view-modal"></i></div><span>板块视图</span></a>
                </div>
              </div>
            </div>
          </footer>
          <div class="forum-write"><a href="javascript:newFocum();"><i class="fa fa-pencil-square-o"></i></a></div>
        </div>
      </div>
    </div>
  </div>
  ...
  $$(".footer-focum>a").click(function(){
   ...
   //进入页面开始加载论坛帖子列表数据
	...
	function loadNextPage(setFlag){
		...
		var url = '/defaultroot/post/pagelist.controller?curPage='+curPage+'&orderType=1';//分页查询的链接
	...
	function loadNextPageNew(setFlag){
		...
		var url = '/defaultroot/post/pagelist.controller?curPage='+curPageNew+'&orderType=2';//分页查询的链接
		//myApp.showPreloader('正在加载中...');//加载提示语
		$.ajax({
			type: 'post',
			url: url,
	...
	function loadNextPageCream(setFlag){
		...
		var url = '/defaultroot/post/pagelist.controller?curPage='+curPageCream;//分页查询的链接
		//myApp.showPreloader('正在加载中...');//加载提示语
		$.ajax({
			type: 'post',
			url: url,
			dataType: 'text',
			data : {'queryTitle' : searchTitle,'queryForumType' : '1'},
		    success: function(data){
				//alert("1111111111111");
				var jsonData = eval("("+data+")");
				//alert("222222222222222");
