<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%><%@ page import="com.whir.common.util.*"%><%@ page import="java.util.*"%>
<%! 
 //获取文件办理url
...
%><% 
	...
	//ezFLOW  任务id 
	String ezflowtaskId =request.getParameter("ezflow_taskId")==null?"":request.getParameter("ezflow_taskId").toString();
	//ezFLOW  流程实例id
	String ezflowprocessInstanceId=request.getParameter("ezflow_processInstanceId")==null?"":request.getParameter("ezflow_processInstanceId").toString();
	//头像
	...
	com.whir.webplatform.manage.bd.WeiXinPlatformBD  wbd=new com.whir.webplatform.manage.bd.WeiXinPlatformBD();
	Object []workInfoObj=null;
	try {
		workInfoObj=wbd.getWorkInfo(isezflow, workId,ezflowtaskId,ezflowprocessInstanceId,request,session);
	} catch (Exception e) { 
		e.printStackTrace();
	}
